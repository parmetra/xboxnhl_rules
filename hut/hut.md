...
1.2.2.1 В зависимости от месяца начала турнира определяется максимальный OVR команды. Этот OVR фиксируется перед стартом сезона и действует, пока не завершится сезон.

1.2.2.2 Если турнир проводится не в актуальной версии игры, то применяются ограничения типа E.

1.2.2.3 Состав команды разрешается менять по ходу турнира. Главное, чтобы он не противоречил пунктам выше текущего.

#### 1.2.3 Требования к арене

1.2.3.1 Команды обязаны использовать в качестве домашних арен площадки канадского типа (размер NHL).

1.2.3.2 Площадки европейского (олимпийского) типа, а также уличные локации не допускаются к использованию в рамках турнира.
