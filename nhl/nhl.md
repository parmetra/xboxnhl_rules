...
1.6.2 После того, как определён порядок выбора команд, первому игроку, который должен выбирать команду, даётся 5 минут, чтобы озвучить своё решение в чате. Если же он не успевает по любой причине, то очередь переходит к следующему, а этот игрок отправляется в конце очереди.
